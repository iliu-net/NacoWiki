...
class ParsedownExtension extends ParsedownToC {
  ...
  */
  ...
  protected function blockFencedCodeComplete($Block) {
    ...
    } elseif ($hclass == 'language-lineart' || $hclass == 'language-bob' || $hclass == 'language-aafigure') {
  ...
  // From: https://www.taeluf.com/blog/php/add-target-blank-to-parsedown-links/
  // If link has a tailing "^" we open in a new window.
  protected function inlineLink($excerpt){
      $link = parent::inlineLink($excerpt);
      if (!isset($link))return null;

      if (substr($link['element']['attributes']['href'],-1) == '^') {
	$link['element']['attributes']['href'] = substr($link['element']['attributes']['href'],0,-1);
